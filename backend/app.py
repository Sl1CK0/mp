...
#         # Load existing data and add new entry
#         dataframe = load_dataframe_from_redis()
#         new_entry = pd.DataFrame([[name, roll_no, role, username, password, embedding, user_id]], 
#                                  columns=['Name', 'RollNo', 'Role', 'Username', 'Password', 'embedding', 'user_id'])
#         dataframe = pd.concat([dataframe, new_entry], ignore_index=True)
...
#     if user_row.empty or user_row['Password'].values[0] != password:
#         return jsonify({"error": "Invalid username or password"}), 400

#     user_id = user_row['user_id'].values[0]
#     attendance_history = r.lrange(f"{user_id}_attendance_dates_times", 0, -1)
...
# def view_attendance():
#     data = request.get_json()
#     roll_number = data.get('rollNo', '')
#     name = data.get('name', '')

#     if not roll_number and not name:
#         return jsonify({"error": "Please provide either roll number or name"}), 400

#     # Fetch attendance records from Redis based on the roll number or name
#     attendance_records = []
#     previous_dates_times = []

#     # If name is provided, try to fetch the record for that name and date
#     if name:
#         today_date = "2024-11-12"  # This should ideally come from the current date
#         redis_key = f"{name}-{today_date}"
        
#         # Fetch data from Redis
#         redis_data = r.get(redis_key)

#         if redis_data:
#             record = json.loads(redis_data)
...
#             "previous_dates_times": previous_dates_times
#         })
#     else:
